...
import EditModal from "@/app/admin/makenotification/components/EditModal";
...
const MakeNotification = () => {
    ...
    const {data: noticeToAll, refetch} = useQuery({
        queryKey: ['noticeToAll'],
        queryFn: async function () {
            const token = localStorage?.getItem('adminToken')
            const response = await axios.get('https://www.cowork.v1r.ir/api/v1/notices', {
                headers: {
                    'Accept': 'application/json',
                    'Authorization': `Bearer ${token}`
                }
    ...
    const makeNotice = async () => {
        ...
        const token = localStorage?.getItem('adminToken')
        try {
            const response = await axios.post('https://www.cowork.v1r.ir/api/v1/notices', {
                "text": text,
                "date": date,
                "title": title
            }, {
            ...
            Swal.fire({
                title: "ارسال شد",
                text: `با موفقیت برای تاریخ فرستاده شد${date}`,
                icon: "success",
    ...
    const deleteById = async (id: number,title:string) => {
        setDeleteLoading(true)
        const token = localStorage?.getItem('adminToken')
        try {
            const response = await axios.delete(`https://www.cowork.v1r.ir/api/v1/notices/${id}`,
                {
                    headers: {
                        'Accept': 'application/json',
                        'Authorization': `Bearer ${token}`
            ...
            {protect ?
                <div className={'w-[100%] flex'}>
                    <div className={'flex flex-col w-[81.25%]'}>
                        <NavbarAdminPanel/>
                        <div className={'w-[100%] min-h-[1626px] pb-5 flex justify-center'}>
                            <div
                                className={'w-[88.97%] flex flex-col pb-5 min-h-[1288px] mt-[56px] rounded-[12px] bg-[#FFFEFF]'}>
                                <div className={'w-[100%] flex justify-end mt-[24px]  pr-[38px]'}>
                                    <p className={'text-[#000000] text-lg font-medium pr-[10px]'}>ایجاد اعلانات برای
                                        کاربران</p>
                                ...
                                <div className={'flex justify-end mt-[8px] pr-[67px]'}>
                                    <p className={'text-neutral-600 text-base font-normal'} dir={'rtl'}>
                                        کاربر محترم شما می توانید در این بخش برای تمامی کاربران این سامانه یک اعلان
                                        ارسال کنید
                                    </p>
                                </div>
                                <div className={'flex  mt-[40px] mx-[67px] justify-between'}>
                                    <div className={'w-[39.57%]'}>
                                        ...
                                        <textarea onChange={(e) => setText(e.target.value)}
                                                  value={text}
                                                  className={'w-[100%] px-4 pt-1.5 text-black mt-[8px] h-[40px] border-[#C4C4C4] bg-[#FFFEFF] border-[1px] rounded-[5px]'}
                                                  dir={'rtl'} placeholder={'متن اعلان را وارد کنید'}/>
                                    </div>
                                    <div className={'w-[39.57%]'}>
                                        ...
                                        <input
                                            ...
                                            value={title}
                                            className={'w-[100%] px-4 mt-[8px] text-black h-[40px] border-[#C4C4C4] bg-[#FFFEFF] border-[1px] rounded-[5px]'}
                                            dir={'rtl'} placeholder={'تیتر اعلان را وارد کنید'}/>
                                    </div>
                                </div>
                                <div className={'flex  mt-[35px] mx-[67px] justify-between'}>
                                    <div className={'w-[39.57%]'}>
                                        ...
                                        <DatePicker
                                            ...
                                            //@ts-ignore
                                            onChange={convert}
                                            inputClass="w-[100%] px-4 mt-[8px] text-black h-[40px] border-[#C4C4C4] bg-[#FFFEFF] border-[1px] rounded-[5px]"
                                            calendar={persian}
                                            locale={persian_fa}
                                            calendarPosition="bottom-center"
                                        />
                                    </div>
                                    <div className={'w-[39.57%] opacity-50'}>
                                        <p className={'text-black text-sm font-normal'} dir={'rtl'}>ارسال اعلان از
                                            طریق</p>
                                        <select
                                ...
                                <div className={'flex mr-[67px] mt-[36px] justify-end'}>
                                    <button
                                        ...
                                        className={'disabled:opacity-50 w-[25.45%] h-[40px] rounded-[5px] flex justify-center items-center bg-[#0075FF] text-stone-50 text-base font-normal'}>
                                        {sendLoading ? <span>در حال ارسال</span> : <span>ارسال</span>}
                                    </button>
                                </div>
                                ...
                                <div className={'w-[100%] flex justify-end mt-[52px]  pr-[38px]'}>
                                    <p className={'text-[#000000] text-lg font-medium pr-[10px]'}>اعلانات موجود در
                                        سایت</p>
                                    <div className={'relative flex items-center'}>
                                        <svg width="18" height="21" viewBox="0 0 18 21" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                ...
                                <div className={'flex justify-end mt-[8px] pr-[67px]'}>
                                    <p className={'text-neutral-600 text-base font-normal'} dir={'rtl'}>
                                        کاربر محترم شما می توانید در این بخش اعلاناتیکه هم اکنون در سایت مووود می باشد
                                        را مشاهده
                                        ، ویرایش و حذف کنید
                                    </p>
                                </div>
                                <div className={'flex flex-wrap mt-[36px] justify-between mx-[67px]'}>
                                    {noticeToAll?.length===0? <div className={'w-full mt-10 flex justify-center items-center text-black text-large font-base self-center'}>هنوز اعلانی ارسال نکردید</div>:
                                        //@ts-ignore
                                        noticeToAll?.map(item => (
                                            <div key={item.id}
                                                 className={'w-[40%] h-[197px] flex mt-5 rounded-[12px] bg-[#F8F9FC]'}>
                                                <div className={'w-[80%] flex items-end flex-col'}>
                                                    <p className={'text-[#002256] mt-[24px] text-base font-bold'}
                                                       dir={'rtl'}>{item.title}</p>
                                                    <p className={'text-[#393939]  w-[271px] mt-[8px] pl-2 overflow-y-auto   min-h-[110px] text-sm font-normal'}
                                                       dir={'rtl'}>{item.text}</p>
                                                    <div className={'flex  items-center justify-start w-[90%]'}>
                                                       <EditModal id={item.id} refetch={refetch}/>
                                                        {deleteLoading ? <div className={'text-center text-[#F53C2B] ml-10'}>...</div> :
                                                            <svg onClick={() => deleteById(item.id, item.title)}
                                                                 className={'ml-[31px] cursor-pointer'} width="24"
                                                                 height="24"
                                                                 viewBox="0 0 24 24"
                                                                 fill="none"
                                                                 xmlns="http://www.w3.org/2000/svg">
